...
<body>
	...
	<form method="post">
		...
		<select name="studentnumber" id="studentnumber">
			...
			$query = "SELECT studentnumber FROM enrolled_students WHERE year = 3 AND schoolyear = 2021 AND studentnumber NOT IN (SELECT studentnumber FROM enrolled_students WHERE year = 4)";
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (isset($_POST['studentnumber'])) {
			...
			if ($enrollmentRow) {
				...
				echo '<form method="post" action="connect4th.php">';
